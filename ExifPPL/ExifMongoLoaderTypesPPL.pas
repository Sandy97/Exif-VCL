...
type
  ...
  public
    ...
    procedure SetConnectionParams(const dbname: string; collections: array of string;
...
//      const insert: boolean): string;
    function StoreToDB(const keynfo: TFileVolBinded;
        const db: string; colnames: array of string; const insert: boolean): string; virtual; abstract;
  end;

    TExifCoupledLoader = class(TExifMongoLoader)
      function StoreToDB(const keynfo: TFileVolBinded;
        const db: string; colnames: array of string; const insert: boolean): string; override;
    end;
...
begin
  ...
  SetConnectionParams(DefaultDBName,[DefaultCollection],false,(poBatch in PartOptions));
  if FBatchMode and (FBCol <> nil) then
    FBCol.BeginBulk(False);
end;
...
procedure TExifMongoLoader.SetConnectionParams(const dbname: string;
...
begin
  ...
  FBatchMode:=batch;
  //LConn.Connected := True;
  FCon := TMongoConnection(LConn.CliObj);
  FEnv := FCon.Env;
...
function TExifCoupledLoader.StoreToDB
  (const keynfo: TFileVolBinded; //Connect: TFDConnection;
  const db: string; colnames: array of string; const insert: boolean): string;
var
  // keyDoc,
...
function TExifSingleLoader.StoreToDB(const keynfo: TFileVolBinded; //Connect: TFDConnection;
             const db: string; colnames: array of string; const insert: boolean): string;
var
  outDoc: TMongoDocument;
  name_: string;
