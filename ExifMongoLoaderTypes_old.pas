...
type
  ...
  TExifCustomLoader = class
    //    FPartOptions: TPartOptSet;
    //    constructor Create(PartOptions: TPartOptSet);
    //    property PartOptions: TPartOptSet;
  end;

  TExifMongoLoader = class(TExifCustomLoader)
  private
    // specific to MongoDB
    FEnv: TMongoEnv;
    ...
    FCollection: string;
    oDoc,
    iDoc, sDoc, mkDoc: TMongoDocument;
  ...
  public
    ...
    // specific to MongoDB
    procedure SetConnectionParams(Connect: TFDConnection;
      const dbname: string; collection: string; const insert, batch: boolean);
    ...
    // вместо строк внизу
    function ToMongoDB(Connect: TFDConnection; const insert, batch: boolean): string;
  end;
  ...
  TExifSingleLoader = class(TExifMongoLoader)
    function StoreToDB(const keynfo: TFileVolBinded; Connect: TFDConnection;
      const db, colname: string; const insert, batch: boolean): string;
  end;

  TExifCoupledLoader = class(TExifMongoLoader)
    function StoreToDB(const keynfo: TFileVolBinded; Connect: TFDConnection;
      const db, ScolNm, McolNm: string; const insert, batch: boolean): string;
  end;
...
begin
  Create;
  SetConnectionParams(Connect,DefaultDBName,DefaultCollection,false,false);
...
procedure TExifMongoLoader.SetConnectionParams(Connect: TFDConnection; const dbname: string; collection: string; const insert, batch: boolean);
begin
...
function TExifMongoLoader.toMongoDB(Connect: TFDConnection; const insert,batch:boolean):string;
var
...
function TExifSingleLoader.StoreToDB(const keynfo: TFileVolBinded;
         Connect: TFDConnection; const db, colname: string; const insert,
...
function TExifCoupledLoader.StoreToDB(const keynfo: TFileVolBinded;
         Connect: TFDConnection; const db, ScolNm, McolNm: string; const insert,
         batch: boolean): string;
var
  mainDoc, cplDoc, keyDoc: TMongoDocument;
  objID: TMongoOID;
  mknPresent: boolean;
begin
  mainDoc:= FEnv.NewDoc;
  ...
  try
    ...
    if insert then begin
      if not Connect.Connected then
        Connect.Connected := True;
      FCon[db][ScolNm].Insert(mainDoc);
      if mknPresent then
        FCon[db][McolNm].Insert(cplDoc);
    end;
  finally
    objID.Free;
    mainDoc.Free;
